...
switch _mode do {
	case "onload": {
		...
		["create", []] call SESO_fnc_iconViewer;
		...
		if (isNil {localNamespace getVariable "HALs_gameIcons"}) then {
			private _h = [] spawn {
				...
				["update", []] call SESO_fnc_iconViewer;
			};
		} else {
			["update", []] call SESO_fnc_iconViewer;
		};
	...
	case "create": {
		...
		_ctrlSearchCheckbox ctrlAddEventHandler ["CheckedChanged", {
			...
			["filterItems", []] call SESO_fnc_iconViewer;
		}];
		...
		_ctrlButtonL ctrlAddEventHandler ["ButtonClick", {["changePage", [-1]] call SESO_fnc_iconViewer;}];
		...
		_ctrlButtonR ctrlAddEventHandler ["ButtonClick", {["changePage", [1]] call SESO_fnc_iconViewer;}];
